...
# NOTE: Some of these things should be done by hand. This means HARDCODE your work.
...
# Under the assumptions of multiple linear regression: 1. There must be a linear relationship between the dependent variable and each independent variable.
...
# However, when running the F test to compare both models, we get a p_value of 0.07713. This is confirmed in
# SAS. Since this value is greater than 0.05, we interpret that the first model, HO, is a statistically better model than H0. 
...
# In the end, we found an F-statistic = 0.67 (which matched the SAS output) and a p_value=0.5176 (matches the SAS output.) Since this p_value > 0.05, we interpret that H0 is a statistically better moel than H1. 
...
#Conclusion - since our pval > 0.05, we fail to reject H0 and conclude that the reduced model is better than
# the model where beta3=beta4.
...
#Conclusion - since our pval > 0.05, we fail to reject H0 and conclude that the reduced model is better than
# the model where beta3+2*beta4 = -17
